...
for sample_id, anno in anno_all.items():
    # load data
    image = imageio.imread(os.path.join(set, 'color', '%.5d.png' % sample_id))
    mask = imageio.imread(os.path.join(set, 'mask', '%.5d.png' % sample_id))
    depth = imageio.imread(os.path.join(set, 'depth', '%.5d.png' % sample_id))
    ...
    ax1.imshow(image)
    ax1.plot(kp_coord_uv[kp_visible, 0], kp_coord_uv[kp_visible, 1], 'r.')
